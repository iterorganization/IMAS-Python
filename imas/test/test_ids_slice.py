...
class TestFlatten:
    ...
    def test_flatten_indexing_and_slicing(self):
        ...
        flattened = cp.profiles_1d[:].ion.flatten()
...
class TestVaryingArraySizeIndexing:
    ...
    def test_unit_slice_element_individual_access(self, wall_varying_sizes):
        ...
        array_0 = element_slice[0]
        assert len(array_0) == 4
        assert array_0[2].name.value == "element-0-2"

        array_1 = element_slice[1]
        assert len(array_1) == 2
    ...
    def test_wall_with_diverse_element_counts(self):
        ...
        assert len(element_slice[0]) == 3
        assert len(element_slice[1]) == 1
        assert len(element_slice[2]) == 4
...
class TestIDSSliceValues:
    ...
    def test_values_integer_and_float_extraction(self):
        ...
        ions = cp.profiles_1d[:].ion.flatten()
    ...
    def test_values_structure_preservation(self):
        ...
        ions = cp.profiles_1d[:].ion.flatten().values()
